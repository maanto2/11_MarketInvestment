...
# ---------------- INDIAN MARKET (NIFTY 50) SUPPORT -------------------
# Separate data store for Indian market
nifty_data_store = {
    "actual": [],
    "predicted": [],
    "timestamps": [],
...
def get_nifty_data(interval='5m', period='7d'):
    """
    Fetch historical NIFTY 50 data using yfinance.
    """
...
def get_nifty_data_for_lstm():
    """
    Get 6 months of daily NIFTY 50 data for LSTM model training.
    Returns:
        pd.DataFrame: Historical daily data for 6 months.
    """
...
def get_nifty_data_for_live():
    """
    Get recent NIFTY 50 data for live prediction (1-minute interval, 7 days).
    Returns:
        pd.DataFrame: Recent minute-level data.
    """
    return get_nifty_data(interval='1m', period='7d')
...
# NIFTY 50 initial training
nifty_hist_daily = get_nifty_data_for_lstm()
nifty_model_lstm, nifty_scaler_lstm = train_lstm_model(nifty_hist_daily)
...
# NIFTY 50 CSV functions
def save_nifty_data_to_csv():
    with open('dashboard_data_nifty.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['timestamp', 'actual', 'predicted', 'sentiment_score'])
        for t, a, p, s in zip(nifty_data_store['timestamps'], nifty_data_store['actual'], nifty_data_store['predicted'], nifty_data_store['sentiment_score']):
            a_ = round(a, 2) if isinstance(a, float) and a is not None else a
            p_ = round(p, 2) if isinstance(p, float) and p is not None else p
            s_ = round(s, 2) if isinstance(s, float) and s is not None else s
            writer.writerow([t, a_, p_, s_])
...
# ----------- UPDATE THREAD FOR NIFTY 50 (INDIAN MARKET) -------------
# Indian market holidays (NSE/BSE)
INDIA_MARKET_HOLIDAYS = [
    # Republic Day
    (1, 26),
    # Holi (date varies, not included here)
...
def is_india_market_holiday(dt):
    # Fixed-date holidays
    if (dt.month, dt.day) in INDIA_MARKET_HOLIDAYS:
        return True
    # Republic Day: Jan 26
    ...
    if dt.month == 12 and dt.day == 25:
        return True
    # Good Friday, Holi, Diwali, etc. (use holidays package if available)
    try:
        import holidays
        india_holidays = holidays.India(years=dt.year)
        if dt in india_holidays:
            return True
    except ImportError:
        pass
    return False
...
def update_nifty_data():
    global nifty_model_lstm, nifty_scaler_lstm
    retrain_interval = 1440
    cycle_count = 0
    while True:
        try:
            cycle_start = time.perf_counter()
            # Indian market open/close times (09:15 to 15:30 IST, Mon-Fri)
            ist = pytz.timezone('Asia/Kolkata')
            india_now = datetime.now(ist)
            is_weekday = india_now.weekday() < 5
            if is_india_market_holiday(india_now.date()):
                time.sleep(300)
                continue
            open_time = dtime(9, 15)
            close_time = dtime(15, 30)
            if is_weekday and open_time <= india_now.time() <= close_time:
                hist = get_nifty_data_for_live()
                sentiment = fetch_sentiment_india_news()
                prediction_lstm = predict_lstm(nifty_model_lstm, nifty_scaler_lstm, hist)
                sentiment_weight = (hist['Close'].iloc[-1] * 0.005)
                prediction = prediction_lstm + sentiment * sentiment_weight
                actual = hist["Close"].iloc[-1]
                now = dt.datetime.now()
                next_timestamp = now + dt.timedelta(minutes=1)
                nifty_data_store["timestamps"].append(next_timestamp)
                nifty_data_store["predicted"].append(prediction)
                nifty_data_store["actual"].append(None)
                nifty_data_store["sentiment_score"].append(sentiment)
                save_nifty_data_to_csv()
                if len(nifty_data_store["actual"]) > 1 and nifty_data_store["actual"][-2] is None:
                    nifty_data_store["actual"][-2] = actual
                    save_nifty_data_to_csv()
                cycle_count += 1
                if cycle_count % retrain_interval == 0:
                    print("Retraining NIFTY LSTM model with latest data...")
                    nifty_hist_daily = get_nifty_data_for_lstm()
                    nifty_model_lstm, nifty_scaler_lstm = train_lstm_model(nifty_hist_daily)
                    print("Retraining complete.")
            else:
            ...
            print(f"[NIFTY Update Thread Error] {e}")
        cycle_elapsed = time.perf_counter() - cycle_start
        logging.info(f"update_nifty_data cycle took {cycle_elapsed:.3f} seconds")
        time.sleep(60)

# ---------------- DASH APP (DUAL MARKET) ------------------
app = Dash(__name__)
app.title = "S&P 500 & NIFTY 50 Prediction Dashboard"

app.layout = html.Div([
    html.H1("Live S&P 500 & NIFTY 50 Prediction Dashboard"),
    html.Div([
        html.Div([
            html.H2("S&P 500"),
        ...
        html.Div([
            ...
            html.Div(id='india-market-status-banner', style={'backgroundColor': '#222', 'color': 'yellow', 'padding': '8px', 'fontWeight': 'bold', 'fontSize': '18px', 'overflowX': 'auto', 'whiteSpace': 'nowrap'}),
            dcc.Graph(id='nifty-trend-graph'),
            html.Div(id='india-news-summary', style={'backgroundColor': '#111', 'color': 'white', 'padding': '12px', 'marginTop': '10px', 'maxHeight': '200px', 'overflowY': 'auto', 'fontSize': '15px', 'border': '1px solid #333'}),
...
@app.callback(
    Output('india-market-status-banner', 'children'),
    [Input('interval', 'n_intervals')]
)
def update_india_market_status(n):
    ist = pytz.timezone('Asia/Kolkata')
    india_now = datetime.now(ist)
    is_weekday = india_now.weekday() < 5
    open_time = dtime(9, 15)
    close_time = dtime(15, 30)
    if is_weekday and open_time <= india_now.time() <= close_time:
        return f'🟢 Indian Market is OPEN | IST: {india_now.strftime("%Y-%m-%d %H:%M:%S %Z")}'
    else:
        return f'🔴 Indian Market is CLOSED | IST: {india_now.strftime("%Y-%m-%d %H:%M:%S %Z")}'
...
@app.callback(
    Output('india-news-summary', 'children'),
    [Input('interval', 'n_intervals')]
)
def update_india_news_summary(n):
    sentiment = fetch_sentiment_india_news()
    return f"Latest Indian Market Sentiment Score: {sentiment:.2f}"
...
def monitor_execution_time(max_seconds=50):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            start = time.perf_counter()
            ...
            if elapsed > max_seconds:
                logging.warning(f"[PERF] {func.__name__} took {elapsed:.2f}s (exceeds {max_seconds}s)")
            else:
                logging.info(f"[PERF] {func.__name__} took {elapsed:.2f}s")
            return result
        return wrapper
    return decorator

# Apply to main update functions
update_data = monitor_execution_time(50)(update_data)
update_nifty_data = monitor_execution_time(50)(update_nifty_data)
